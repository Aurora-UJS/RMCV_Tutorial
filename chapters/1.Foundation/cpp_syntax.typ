...
浮点类型用于表示带有小数部分的数值，在机器人控制中随处可见：角度、速度、比例—积分—微分（Proportional–Integral–Derivative，PID）控制器参数、坐标位置等都需要用浮点数表示。这里的 PID 与上一章表示进程号的 PID（Process ID）不是同一术语。
...
在采用 IEEE 754 的常见平台上，`float` 通常占 4 字节并提供约 6～9 位十进制有效数字，`double` 通常占 8 字节并提供约 15～17 位；标准允许实现有所不同。`float` 能减少数组和图像数据的带宽，某些 GPU、单指令多数据（Single Instruction, Multiple Data，SIMD）向量单元或嵌入式硬件也更擅长处理它；`double` 则能降低许多累计计算中的舍入影响。应根据误差预算、数据规模和目标硬件测量选择，而不是只凭“精度更高”或“占用更小”一条理由。
...
从接口写法看，C++ 函数参数常见值、引用和指针三种形式；严格来说，指针本身仍然按值传入，只是它保存了对象地址。选择时要回答三个问题：函数是否需要修改调用者的对象，“没有对象”是不是一种合法输入，以及复制这个类型的成本是否值得。

值传递是默认方式。调用函数时，形参由实参初始化：传入左值时通常复制，传入右值时还可能移动，并受拷贝消除等规则影响。无论具体怎样构造，函数内部修改的都是自己的形参，不会把调用者原来的那个对象直接改掉：
...
函数可以调用自身，这称为递归。树、目录和分治问题本身就由更小的同类问题组成，用递归描述通常比较直接：
...
这个简化类的 `operator[]` 不检查边界，而且默认复制会复制裸指针，暂时还不是一个完整可用的容器；下方预览会展示复制问题，下一节再补齐对象语义。
...
继承后，基类成员能否由派生类访问、又能否暴露给外部代码，取决于访问级别。先看 `public`、`private` 和 `protected` 各自控制什么：
...
当调用方只需要“处理目标”这一共同能力，而具体算法随目标类型变化时，多态可以把稳定接口和各自实现分开。下面看几个 RoboMaster 相关的例子。
...
这仍是讲解接口关系的教学类型，工程代码通常应先考虑标准库的 `std::complex`。真正值得带到下一节的不是“尽量多重载几个符号”，而是契约意识：短语法也要说明修改对象与否、边界在哪里、失败后对象处于什么状态。标准模板库（Standard Template Library，STL）的容器正是下一组这样的契约——它们把元素的存放方式、访问方式和迭代规则组织成几类可组合的接口。
...
到这里已经足以读懂常见容器与算法的模板外形。替换失败并非错误（Substitution Failure Is Not An Error，SFINAE）、类型萃取和更深的元编程可以按项目需要继续学习。下一节转向一个具体的类模板家族：智能指针怎样把动态对象的所有权和销毁动作纳入 RAII。
...
`std::shared_ptr<T>` 让多个指针对象共同拥有一个动态对象。它们共享控制块，其中记录强所有者等信息；最后一个强所有者释放后，被管理对象才会销毁。下面把惯性测量单元（Inertial Measurement Unit，IMU）只当作一个传感器名字，示例关注的是共享所有权，不展开它的测量模型。
...
扩展卡尔曼滤波（extended Kalman filter，EKF）和弹道模型同样能用 Eigen 实现，却不适合用几十行“完整类”制造已经可用的印象。EKF 还要处理雅可比、噪声离散化、角度残差、协方差对称性与正定性，更新时通常避免显式求逆并考虑 Joseph 形式；弹道则要验证可达性、单位、阻力模型、积分误差和根的分支。Eigen 负责矩阵计算，不验证这些模型假设。
...
边界只是搜索域，不是校准证据。结果贴在边界上可能表示数据偏好边界外、初值/尺度有问题，或模型不可辨识，应作为诊断信号。固定一部分坐标可用 `SubsetManifold`，但先要认清问题中的规范自由度（gauge freedom）：如果整体平移、旋转或尺度等参考系变换不会改变观测模型的预测，多组参数就能同样解释数据，优化器无法自行选出唯一基准。具体固定哪些自由度，必须与实际的物理参考系一致。
...
==== RoboMaster 应用：透视 n 点（Perspective-n-Point，PnP）问题

PnP 在已知相机内参和三维点—二维像素对应时求相机位姿。实际工程常先用 OpenCV 的 PnP 求解器与随机采样一致性（Random Sample Consensus，RANSAC）获得初值和内点，再用 Ceres 在明确的内点集合上细化；局部优化器不负责从任意初值中发现正确姿态。
