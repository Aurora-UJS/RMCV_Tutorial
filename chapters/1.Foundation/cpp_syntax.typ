...
OpenCV 是计算机视觉领域的事实标准库，RM 视觉程序里从读相机、找灯条到解算位姿，几乎每一行都要和它打交道。本节不求全（它有两千多个函数），只讲最核心的数据结构 `cv::Mat` 和一条完整的识别流水线。特别说明：本节所有插图都不是示意图，而是用本节代码在一段真实分区赛视频（仓库 `examples/test.mp4`）上跑出来的结果，完整源码在 `examples/cpp/` 下（示例中的相对路径都以该目录为工作目录），你可以逐行复现。编译只需一行（CMake 工程里用 `find_package(OpenCV)`，见“CMake 与构建系统”一章）：

```bash
g++ -O2 demo.cpp -o demo $(pkg-config --cflags --libs opencv4)
```

==== cv::Mat：既是图像，也是一份“智能指针”

`cv::Mat` 是 OpenCV 的图像容器，它的内存管理方式你其实已经学过了——就是智能指针一节的引用计数思想。`Mat` 分为两部分：轻量的头（尺寸、类型、数据指针）和真正的像素数据。赋值和拷贝构造只复制头、共享数据、引用计数加一；数据在最后一个使用者销毁时才释放：

```cpp
cv::Mat a = cv::imread("frame.png");  // a 拥有像素数据
cv::Mat b = a;          // 浅拷贝：b 和 a 共享同一块像素，代价近乎为零
b.at<cv::Vec3b>(0, 0) = {0, 0, 255};  // 通过 b 修改，a 也变了！
cv::Mat c = a.clone();  // 深拷贝：c 有独立的数据副本，改 c 不影响 a
```

这个设计让 `Mat` 可以放心地按值传参、按值返回（想想移动语义一节的讨论——这里连移动都省了），但也埋着新手最常撞的坑：想备份一帧图像却写了 `backup = frame`，结果“备份”跟着原图一起被下一帧覆盖。*记住口诀：赋值是别名，`clone` 才是副本。*

ROI（Region of Interest，感兴趣区域）同样基于共享：`frame(cv::Rect(x, y, w, h))` 返回的子图和原图共用像素，往子图上画框就是往原图上画框。像素类型用 `CV_8UC3` 这样的代号表示——8 位无符号、3 通道，正是最常见的彩色图。像素按行主序连续存储，所以遍历图像要外层行、内层列——“计算机系统基础”一章按行扫比按列扫快二十多倍的实验，说的就是它。

==== 读取图像与视频

静态图像用 `imread`/`imwrite`，调试时配合 `imshow` + `waitKey` 弹窗看图。视频和相机都走 `cv::VideoCapture`：传文件路径就读视频，传设备号 `0` 就开相机。本节的素材帧是这样取出来的：
...
这一帧的画面：中景两台蓝方车——前面一台装甲板灯条清晰可见（它是什么兵种，后文分类器会揭晓），身后半遮挡着一台 3 号步兵；近景是一台红方车的尾部灯条，再加白色数字板和场地灯光当干扰——正好是一道识别习题。

==== 颜色空间：该在哪个空间里找灯条

先记住一个容易翻车的约定：OpenCV 的默认通道顺序是 *BGR 而不是 RGB*——`Vec3b` 的第 0 个分量是蓝。颜色空间转换用 `cvtColor`，通道拆分用 `split`：
...
把前面那台蓝方车区域的四种视图摆在一起看：

#figure(
  image("images/cpp-opencv-colorspaces.png", width: 100%),
  caption: [同一画面的四种视图。V 通道里灯条最亮最稳；H 通道整体是一片模糊的噪声——低饱和度像素的色相数值毫无意义。结论：用亮度找发光体，用颜色分红蓝。],
)

这张图值得多看几眼。教科书常说“HSV 把颜色和亮度解耦，按 H 阈值就能选颜色”，但真实赛场画面里灰色地板、黑色车体的饱和度都接近零，此时色相是纯噪声——图中 H 通道几乎看不出任何结构，灯条也毫不突出。反倒是灰度图和 V 通道里，灯条作为全场最亮的东西一目了然。这就是工程和教科书的区别：*亮度信息负责“哪里有发光体”，颜色信息只负责“它是红是蓝”*。那红蓝到底怎么分？工程上通行的答案很优雅：不做任何颜色阈值，先把所有发光体一视同仁找出来，再对每个发光体内部统计 B 通道与 R 通道的像素值总和，谁大判谁的颜色。这套投票对成像条件很鲁棒：实车低曝光画面里灯条中心常过曝成白色，白芯 B≈R 互相抵消，胜负由斑块内其余带色像素说了算；本节这段普通曝光素材里斑块本身就偏蓝或偏红，投票赢得更直接。下一小节就按这套思路走完全程。

==== 实战：在比赛画面里找出装甲板

现在把零件组装成完整流水线。下面这套四级级联是社区多年沉淀出的经典方案——各强队的传统视觉方案大同小异，每一级只回答一个问题：

- *二值化*——哪里亮？灰度图按亮度阈值二值化，红蓝灯条统统变成白色斑块，这一步完全不碰颜色。
- *找灯条候选*——哪些亮斑长得像灯条？取轮廓的最小外接旋转矩形，要求细长（宽长比 0.1～0.4）且基本竖直（倾角小于 40 度），然后逐候选做红蓝投票。注意“候选”二字：这一级只负责宽进，混进来的杂物交给后面收拾。
- *配对*——哪两根灯条组成装甲板？只在*敌方颜色*的灯条候选里两两尝试（自瞄不打友军，本例设我方为红方、只配蓝色），要求等长（长度比大于 0.7）、连线近水平（倾角小于 35 度）、间距合适。间距以灯条长度为尺子归一化：0.8～3.2 倍是小装甲板，3.2～5.5 倍是大装甲板——同一条判据顺便区分了板型。另有一条否决规则：两根灯条之间不许夹着第三根灯条。
- *分类器*——框出来的真是装甲板吗？把两灯条之间的区域透视矫正成 20×28 小图，交给 MLP 网络认图案：数字、哨兵、前哨站等图案放行，“负样本”淘汰。几何规则宽进，分类器严出。

核心判据的代码就几行（完整可运行版见 `examples/cpp/lightbar_pipeline.cpp`）：

```cpp
// 灯条几何判据：细长且基本竖直
bool is_light(const Light& l) {
    float ratio = l.width / l.length;       // 宽长比
    return 0.1f < ratio && ratio < 0.4f && l.tilt_angle < 40.f;
}

// 颜色投票：轮廓内 B、R 像素值之和，谁大就是谁
sum_b += px[0];  sum_r += px[2];            // BGR 顺序
light.color = sum_b > sum_r ? BLUE : RED;

// 装甲板几何判据（节选）：间距以灯条长度归一化
float dist = cv::norm(a.center - b.center) / avg_len;
bool small_ok = 0.8f <= dist && dist < 3.2f;   // 小装甲板
...
先看第 3 块图：11 个灯条候选里有 9 根是真灯条——红车尾部 3 根、前面蓝车两块装甲板 4 根、后方步兵 2 根；混进来的 2 个“杂物”，一个是步兵装甲板上数字图案的发光笔画，一个是蓝车装甲板旁的一小条反光。这不是判据写错了：在这段普通曝光的观赛视频里，这些杂光和真灯条的宽长比、倾角、亮度几乎完全重叠——它们能通过同一组几何判据本身就是证明，单帧几何信息原理上就分不开。实车自瞄相机用 1～4 毫秒的低曝光拍摄，画面里除了自发光的灯条几乎全黑，这组判据正是为那个域设计的——*视觉算法的判据永远和成像条件绑定*，这是比任何参数都重要的一课。好消息是颜色投票对全部 11 个候选都判对了阵营。

再看级联怎么收拾这摊杂物：近景那台红方车是“我方”，从一开始就不参与配对——画面里它再显眼也不会有候选框；几何配对在剩下的蓝色候选里只放行 4 个组合，其中一个跨装甲板的错误组合、一个“真灯条＋邻近杂光”的组合，被分类器以“负样本 100.0%”当场处决。幸存的两个都被判为 guard（哨兵图案），置信度 99.9%——两块装甲板独立得出同一判断，这台蓝方车是哨兵无疑。没有哪一级是完美的，但每一级都在削减上一级的错误——这就是级联设计的全部哲学。

还有一个值得玩味的细节：后方那台步兵的装甲板，两根真灯条其实通过了全部装甲板几何判据，却在配对阶段被“不许夹灯”规则否决了——数字图案的发光笔画恰好也通过了灯条判据（宽长比 0.39、倾角 9.5 度，和真灯条难分彼此），正夹在两根真灯条中间。一条为拦住跨板错误组合而生的规则，在这里误伤了一块真装甲板，它从头到尾没能成为候选。一条规则挡住一类错误，也误伤一个正确答案——单帧几何规则的两难在这里一览无余，实战系统靠连续帧跟踪（tracker）来兜底——这正是成熟自瞄框架里跟踪模块存在的理由。

把镜头拉远一点看：误识别不是这段素材的特例，而是 RM 视觉的*常态*。即使在实车的低曝光域里，场馆灯带与大屏、地面反光、残血时闪烁的灯条、被击打熄灭一半的灯条、乃至队友的车灯，都在持续制造假候选；而各兵种装甲板外形几乎一样，“认错车”比“认错板”更隐蔽。归根结底，原因有三层：单帧几何信息本身有歧义（细长亮斑并非灯条专利，本节已经眼见为实）；成像条件在移动对抗中不断波动（曝光、白平衡、运动模糊）；目标自身也在变化（闪烁、遮挡、姿态翻转）。所以识别链条必须不断向后延伸——数字分类器回答“是不是装甲板、是哪台车”；整车建模更进一步，把一台车抽象成绕中心旋转的四块装甲板，用滤波器在时间维度上积累证据：单帧的假候选很难在连续几十帧里自圆其说，对手开小陀螺时还能据此预测下一块板会转到哪里；再往前，不少强队已改用神经网络直接回归灯条角点，把几何判据的模糊性交给数据去学。每一级新手段，都是对“上一级还会犯错”的清醒承认——这条军备竞赛没有终点，理解了这一点，你才算真正入了 RM 视觉的门。

最后交代边界：以上阈值和几何参数都是社区里久经实战检验的经验取值，没有为这一帧做任何特调——分层设计让它天然抗造。检出装甲板只是第一步，接下来要用两灯条的像素几何加相机内参解算三维位姿（PnP），弹道和预测才能出场，那是“数学理论篇”相机模型与坐标变换的主场。这套级联思路有成熟的开源实现可以对照学习（如 rm_vision，本节的分类器模型即取自该项目，MIT 协议），“项目分析”篇会对它做完整拆解。

=== 串口通信

视觉程序算出的云台角度，最终要通过那根 USB 转 TTL 线送进电控的单片机，这条链路就是串口。它的“身份”和“哲学”在“计算机系统基础”一章都已经讲过：串口在 Linux 里是字符设备（`/dev/ttyUSB0`、`/dev/ttyACM0`），打不开先查 `dialout` 组权限；协议层面用帧头加 CRC 校验、坏帧直接丢弃，是“宁要最新、不要完整”的 UDP 哲学。本节把这套思想落成可编译、可运行的 C++ 代码——termios 配置、结构体打包、CRC 校验、逐字节解析状态机，最后在没有任何硬件的情况下把整条链路测通。完整源码在仓库 `examples/cpp/serial_loopback.cpp`。

==== 打开并配置串口

串口既然是文件，第一步自然是 `open`。拿到文件描述符后，用 termios 接口把它配置成“原始模式”：
...
两处最容易踩的坑都在默认行为里。其一，串口设备默认挂着*终端行规程*——内核会把它当成人对着敲键盘的终端，帮你做回显、行缓冲、把 `0x0D` 翻译成换行等各种“贴心”处理，而这些对二进制协议全是灾难：数据流里凑巧出现一个 `0x11`（XON 流控字符），你的字节就莫名消失了。`cfmakeraw` 一次性关掉所有这些翻译，让读到的字节严格等于线上的字节。其二，`open` 时的 `O_NOCTTY` 表示“别把这个串口当成本进程的控制终端”，否则串口上偶然出现的特殊字节可能触发内核向你的进程发信号。

`VMIN` 和 `VTIME` 决定 `read` 的阻塞语义，这里埋着一个 man 手册专门警告过的经典误解，值得花一段讲透。`VMIN=1, VTIME=1` 的真实语义是：阻塞直到至少 1 个字节到达，然后立刻带着已到的数据返回——第 1 个字节本身就满足了 VMIN，VTIME 的字节间计时器根本来不及参与。“字节间隔超时聚帧”是 VMIN 大于 1 时才有的行为（收到首字节后，相邻字节间隔超过 VTIME 就提前返回）。更要紧的是别指望它防卡死：这个计时器要收到第一个字节才启动，对端彻底沉默时（线断了、电控死机——车上最常见的故障），`read` 会无限期阻塞。需要“对方不发数据也定期返回”的看门狗语义，得改用 VMIN=0、VTIME>0，或在 `read` 外面套 `poll()`。本节示例逐字节读取，“阻塞到有数据就返回”正是想要的行为，VMIN=1 刚好够用。

==== 定义帧：结构体就是字节流的模具

协议帧在 C++ 里最自然的表达就是一个结构体。以视觉发给电控的云台指令为例：
...
`#pragma pack(1)` 是这里的生命线。前文讲过编译器会为对齐在成员之间插入填充字节——正常写程序时这是好事，但协议帧要求内存布局和线上字节流一个字节都不能差：不关填充，`head` 后面会被塞进 3 个空字节，帧长变成 16，电控按 11 字节解析直接全错。`static_assert` 在编译期把帧长钉死，将来有人往结构体里加字段忘了改协议，编译器第一时间报错，而不是等到赛场上云台开始抽风。

校验用一个多项式 0x31 的 CRC8，逐位实现只有几行（CRC 为什么能高概率检出损坏，“计算机系统基础”的串口协议一节讲过思想）：
...
还有一个容易被忽略的前提：直接 `memcpy` 结构体收发，隐含了“两端字节序相同”的假设。x86 的 NUC 和 STM32 的 Cortex-M 恰好都是小端，所以这套做法在 RM 的典型链路上成立；如果哪天对接了大端设备或要跨网络传输，就必须逐字段显式序列化。把假设写成注释放在协议定义旁边，是对未来队友的基本礼貌。

==== 接收状态机：粘包、半包与坏帧

发送端一帧一帧地写，接收端却*不会*一帧一帧地收——串口是字节流，`read` 返回的可能是半帧、一帧半、也可能夹着上电噪声。所以健壮的接收端永远是逐字节喂进一个状态机：
...
逻辑只有三步：没同步上就一直丢字节，直到撞见 `0xA5`；收满固定帧长；验 CRC，不过就整帧扔掉重新找头。注意这个“扔掉”正是协议设计哲学的落地——坏了不修、不补传，等 10 ms 后的下一帧。一个留给你的思考题：数据字节里也可能凑巧出现 `0xA5`，如果某帧因此错位，上面的简化实现要多久才能重新同步？提示：答案可能是“永远”——当发送内容稳定不变（比如云台指令进入稳态）而载荷里恰好有个字节等于 `0xA5` 时，状态机每次都会锁到同一个假帧头上，周期性地错下去。更严谨的做法是 CRC 失败后从缓冲区第 2 个字节起重新扫描帧头，感兴趣可以自己改造并构造这种最坏情形验证。

==== 没有硬件也能测：pty 回环

写完收发两端，手边没有电控板怎么验证？Linux 的 `openpty` 可以凭空创建一对互连的虚拟终端：写进一端的字节从另一端原样流出，行为等价于一根回环的串口线。两点交代：新建的 pty 和真实串口一样默认挂着终端行规程，所以第一件事仍是用 `configure_serial` 把它配成 raw 模式；这种等价也只到字节内容为止——pty 不模拟波特率与电气时序，数据瞬间到达，它能测协议逻辑，测不了波特率不匹配、线路噪声这类物理层问题：

```cpp
#include <pty.h>   // 链接时加 -lutil

int master, slave;
openpty(&master, &slave, nullptr, nullptr, nullptr);
configure_serial(slave);

// 发送：一条合法帧 + 两个垃圾字节 + 一条 CRC 损坏的帧
GimbalCmd tx{};
tx.head = 0xA5;  tx.yaw = -12.5f;  tx.pitch = 3.75f;  tx.fire = 1;
tx.crc = crc8(reinterpret_cast<uint8_t*>(&tx), sizeof(tx) - 1);
// ……写入 master 端（完整代码见 examples/cpp/serial_loopback.cpp）

// 接收：从 slave 端逐字节读出，喂给状态机
```
...
三个测试意图全部命中：合法帧被正确解析出发送时的数值；垃圾字节被状态机默默跳过；被篡改过数据但没重算 CRC 的帧被校验拦下。这就是“软件工程基础”一章强调的可测性：因为收发两端只依赖一个文件描述符，测试时用 pty 顶替真实串口，整条链路在笔记本上就能回归验证——上车之前，协议解析的 bug 就该已经死完了。

实际项目中你也会遇到现成的封装：ROS 生态的 serial 包、boost::asio 的串口类，都把 termios 细节包了起来。用它们没问题，但排障时终究要回到本节的底层认知——波特率对不对、行规程关没关、权限有没有、帧有没有对齐。最后两件上车实操的事回顾一下：打不开设备先想 `dialout` 组（“计算机系统基础”讲过），设备名在 `ttyUSB0`/`ttyUSB1` 之间跳变则用 udev 规则固定，后文讲串口模块的章节会给出完整做法。

至此，这一章可以收束了。前半程是语言本身：从变量和函数，到面向对象，再到模板、智能指针、移动语义与多线程，这些构成了编写现代 C++ 代码的地基。后半程把地基接上了 RM 的实际链路：用 Eigen 与 Ceres 做数学运算和优化，用 OpenCV 在真实比赛画面里识别装甲板，再经串口把云台指令送进电控。从一帧图像到一条指令，你已经可以用 C++ 把整条链路走通——接下来的章节，就是把这条链路的每一站做深做稳。
