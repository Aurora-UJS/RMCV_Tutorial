...
后面的 `rclcpp::spin(node)` 是便捷写法，默认使用单线程执行器。需要让多个回调并行时，可以显式使用 `MultiThreadedExecutor`。

不过，仅更换执行器并不保证吞吐量提高：回调组会限制哪些回调可以同时运行，共享状态也必须正确同步。设计节点边界时，应把“通信接口怎样划分”与“回调在哪些线程执行”分开考虑，并在目标负载下测量延迟。
...
如果预期的连接没有出现，先检查节点和话题名称。若图中存在相同话题的发布者与订阅者，却没有实际消息，可以继续执行：

```bash
ros2 topic info -v /detector/target
```

这条命令用于检查双方的消息类型与 QoS；还要结合日志确认发布节点是否真的在发送消息。
...
当节点进程存在但没有得到预期结果时，可以按下面五层逐步检查；其中频率监测、消息输出和图形工具会持续运行，用完后退出，再进入下一项即可。

1. *节点是否都在*：使用 `ros2 node list`。
2. *关键话题是否存在*：使用 `ros2 topic list`。
3. *频率和内容是否合理*：分别使用 `ros2 topic hz /detector/target` 和 `ros2 topic echo /detector/target`。
4. *发布与订阅关系是否符合设计*：打开 `rqt_graph`。
5. *坐标系和可视化数据是否一致*：打开 `rviz2`。
