...
“过早优化是万恶之源”这句话被引用了无数次，但它几乎总是被引错。Knuth 的原话还有后半句：程序里大约 97% 的时间不值得为效率操心，剩下那 3% 才是关键，而且“我们不该放过那关键的 3%”。这句话真正的意思不是“别优化”，而是——*在你知道哪 3% 是关键之前，别动手*。凭直觉猜热点，猜错的概率高得惊人：几乎每个人都经历过花两天优化一个函数，结果整体快了 0.5% 的故事。

所以性能工作的形状是一个闭环：测量 $arrow.r$ 分析 $arrow.r$ 优化 $arrow.r$ *验证*。最后一步最常被跳过，也最致命——没有验证，你不知道自己是真的优化了，还是只是把开销挪到了别处（比如把一次拷贝换成了一次堆分配）。养成习惯：每次改动前后各测一遍，用同一组输入、同一台机器、同一个构建类型。
...
这里必须用 `steady_clock` 而不是 `system_clock`。后者是墙上时钟，会被 NTP 校时、手动改时间甚至夏令时调整拽着走，两次采样相减完全可能得到负数；`steady_clock` 单调递增，只用来量“过了多久”，这正是我们要的。

单次计时基本没有意义。同一段代码跑一百遍，耗时会散布成一个分布，受缓存冷热、调度抢占、频率调节的影响。更重要的是：*对实时系统而言，平均值是最没用的统计量*。自瞄以 100 fps 运行时，每帧预算 10 ms；一个平均 4 ms、但每 50 帧就有一次 25 ms 的检测器，平均值好看，却每半秒稳定丢一帧。要看的是分布的尾巴——99 分位数、最大值，以及超预算的帧占比。想把这件事做规范，可以用 Google Benchmark 这类基准测试框架，它会自动决定迭代次数、报告统计量，省掉自己写计时循环的琐碎。

至于分析工具——`perf` 怎么用、火焰图怎么读、Valgrind 怎么查内存问题，“计算机系统基础”一章的性能分析与调试工具一节已经完整讲过，还带了一次真实的自瞄掉帧定位过程，这里不重复。

优化方向大体有个优先级，从收益高到收益低：先看算法复杂度（把 $O(n^2)$ 的配对换成有序扫描，胜过任何常数因子上的努力），再看内存访问模式（一次缓存未命中的代价接近命中的百倍，“计算机系统基础”一章算过这笔账），然后是避免不必要的拷贝（移动语义、按引用传大对象；顺带纠正一个常见误解：`cv::Mat` 本身是引用计数的*浅*拷贝，按值传递并不贵，真正贵的是你随手写下的那句 `clone()`）与减少堆分配（预分配、对象复用），最后才是多线程并行——它增加的复杂度和 bug 面积，往往比它省下的时间更贵。编译选项是几乎免费的一档：确认比赛构建用的是 `Release`，这件事在“CMake 与构建系统”一章已经算过账。

以上是方法论，适用于任何 C++ 项目。RoboMaster 特有的那部分——图像流水线上每一段延迟到底花在哪、板卡与推理引擎怎么选、CPU 亲和性与实时优先级要不要动、以及怎么攒出一张端到端的延迟预算表——放在“进阶篇”的性能优化技巧一章，那里有具体的数字和取舍。
