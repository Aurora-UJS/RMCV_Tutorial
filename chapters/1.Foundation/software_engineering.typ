...
命名空间使用小写字母，通常取项目名或模块名的缩写。代码量小时名称冲突不明显；同时引入多个库和模块后，命名空间能说明一个类型属于哪里，也能避免两个同名 `Detector` 落进全局作用域。
...
公开头文件每多包含一个依赖，所有包含者都可能跟着解析它，并在依赖变化后重新编译。组织头文件时要同时处理两件事：防止重复包含，以及只暴露接口真正需要的声明和依赖。
...
迪米特法则也称最少知识原则。它提醒调用者不要沿着很长的对象关系取得内部状态，例如：

```cpp
robot.controller().serial().port().write(...)
```

这种写法会把多层结构暴露给上层。由合适的模块提供意图明确的操作，可以减少内部结构变化向外传播。它并非按“认识的对象数量”机械计数；数据转换、诊断和性能关键路径有时需要更直接的访问，此时应把依赖和稳定性要求写清楚。
...
在 gdb 中，条件断点、观察点和多线程检查可以分别这样写：

```text
break detector.cpp:87 if frame_id == 120
watch variable
info threads
thread apply all backtrace
```

观察点会在内存值变化时暂停；硬件观察点数量有限，软件实现可能很慢。线程列表和调用栈只能说明程序暂停时的位置。附加调试器和断点还会改变线程调度，因此竞态问题仍需要 ThreadSanitizer、重复运行或更有针对性的同步检查。
...
- `CalculateProjectileDropCompensation` 比 `Calc` 更明确。
- `remaining_ammo` 比脱离上下文的 `n` 提供更多信息。

但名称仍无法说明采用的弹道模型、输入单位和无解时的行为，这些内容需要类型、接口约定或文档共同表达。
...
含义不明的数值不该直接散落在代码里。应把它改成有名称、单位和来源的常量或配置。

例如，下面这个名称比孤立的 `5.0` 更能说明用途：

```text
kMaxFiringDistanceMeters
```

若该值来自规则或标定，还应在配置说明或注释中记录依据。
