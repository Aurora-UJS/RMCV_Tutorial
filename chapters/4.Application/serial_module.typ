...
但你手上这台，是*一台步兵*。换成雷达站，相机不再骑在云台上而是架到场地边，任务也从"打中一台车"变成"把整场比赛的敌情标进小地图"；换成哨兵，车自己会跑，视觉得先在自身运动里分辨清楚是我在动还是它在动，还要替决策层回答"先打谁"。这些兵种会把前四篇里许多被默认成真的假设一条条掀翻——那是「进阶篇」要办的第一件事。第二件事是把性能榨干：帧率不够时，钱该花在换板卡、换推理引擎，还是花在少拷贝一次图像上？没有一张端到端的延迟预算表，这个问题只能靠猜。第三件事最不体面也最值钱——把这套系统在赛场上会以哪些方式死掉，一条条归档。
