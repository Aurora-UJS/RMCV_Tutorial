...
本章重点是接收线程与发送回调、ROS 2 接口、断线恢复、稳定设备路径和验证方法。参考实现固定为 `rm_serial_driver` 提交 `794f4ec`，便于后文把观察到的行为与具体源码对应起来。
...
`packed` 通常会去掉成员间填充，但不能统一 C++ 位域分配顺序、`bool` 表示、浮点格式和字节序。常见 x86 主机与 STM32 均使用小端并不足以证明两端布局相同，尤其这里混用了 `bool` 与 `uint8_t` 位域。用本机 g++ 编译所核对头文件得到 28 B 和 48 B，只能证明当前 ABI（应用二进制接口）下的布局。部署时应在两端分别使用 `sizeof`/`offsetof` 或等价静态检查，并用一组固定字段生成已知正确的金样字节（golden bytes），让两端互相解码；需要跨编译器和平台稳定时，应逐字段编码，而不是直接复制对象表示。
...
两种接口对应不同职责划分：在本章核对的 `rm_serial_driver` 固定提交中，发送包把目标整车状态交给下位机；同时核对的 `rm_vision` bringup 快照（提交 `2ead8d0`）只启动并连接相机、识别、跟踪和串口等节点，没有呈现弹道、选板与开火模块。这只能说明所查公开快照的接口边界，不能推断其他版本或完整整车系统的职责。`sp_vision` 固定提交则在上位机生成云台轨迹与开火模式。前一种工程把识别、跟踪和串口组织成 ROS 2 节点，通过话题、TF、参数与服务连接；后一种的普通兵种程序把相机、视觉与 I/O 组织在同一 C++ 进程内，以类、线程和队列连接。选择哪一种取决于控制器能力、反馈信息、时间同步和团队维护边界。

两字节帧头可以降低随机载荷恰好匹配完整帧头的概率，但固定 `Gimbal` 解析器每次读取两个帧头字节，匹配失败后整块丢弃，并不会逐字节滑动。若字节流插入或丢失一个字节，某个真正的 `SP` 可能恰好跨在相邻两次读取之间而被漏掉，恢复时机取决于后续帧长和读取边界；更稳妥的做法是维护持久缓冲区，匹配失败后只滑动一个字节，并测试分片与错位输入。其发送函数也忽略 `serial_.write()` 的返回长度；在默认写超时为 `0` 时，内嵌 serial 库可能在首次底层写入后返回部分长度，因此这条路径同样需要处理部分写入。`static_assert(sizeof(...) <= 64)` 只限制当前编译器下的上限，也不能替代两端金样字节测试。
...
#figure(
  table(
    ...
    [权限], [目标账户可读写，组与访问控制列表（ACL）符合安全策略], [`Permission denied` 或权限过宽],
    ...
    [包布局], [两端 `sizeof`/字段偏移与金样字节互解一致], [编译器或固件更换后字段错位],
