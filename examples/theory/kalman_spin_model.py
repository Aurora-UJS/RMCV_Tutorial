# Whole-vehicle model figure for the aimbot landing section:
...
# right -- why it works: the armor sweeps a looping trochoid while the
#          center goes straight. Track the center, not the plate.
...
# ---------- left: geometry at one instant --------------------------------
r_pair = [0.26, 0.20]                    # two mounting radii (alternate)
...
ax1.plot([0, 0.26 * n0[0]], [0, 0.26 * n0[1]], color="black", lw=1.0)
ax1.annotate("$r$", (0.5 * 0.26 * n0[0] - 0.06, 0.5 * 0.26 * n0[1]),
             fontsize=11)
...
ax1.set_title("One state, four plates", fontsize=11.5)
...
ax2.plot(ax_, ay_, color=BLUE, lw=2.2, alpha=0.8,
         label="armor plate (trochoid)")
ax2.plot(cx, cy, color=MAGENTA, lw=2.6, label="chassis center (straight)")
...
ax2.set_title("Spin 1.5 rev/s + translation 1 m/s:\n"
              "armor loops, center goes straight", fontsize=11.5)
...
fig.tight_layout()
fig.savefig("chapters/2.Theory/images/theory-kalman-spin-model.png",
            bbox_inches="tight")
