# Synthetic big-rune rotation example for the application chapter.
# The upper panel fits a sine model to noisy angular-speed samples. The lower
# panel compares angular lead error with an illustrative image-plane angle band;
# it does not model the complete projectile and control chain.
...
sp_noisy = spd(ts) + rng.normal(0.0, 0.30, ts.size)        # synthetic speed-observation noise

DT = 0.40          # illustrative total lead time
FIT_START = 3.0    # plotting choice, not a general convergence threshold
...
thr = np.degrees(0.5 * 300 / 636.42)   # small-angle reference: half image-plane chord / radius
...
axA.scatter(ts, sp_noisy, s=9, color=GRAY, alpha=0.55, zorder=1,
            label="synthetic noisy speed observations")
...
axA.set_title(f"Synthetic big-rune model: frequency scan with linear least squares   "
...
axB.axhline(-thr, color=GREEN, lw=1.3, zorder=1)
axB.text(FIT_START + 0.08, thr - 0.7,
         f"+/-{thr:.1f} deg image-plane reference band\n"
         "(angular check only; not a hit guarantee)",
         color=GREEN, fontsize=9.1, va="top", ha="left")
...
axB.set_title("Angular lead error under the stated synthetic model",
              fontsize=11.5)
axB.legend(loc="lower right", fontsize=9.0, frameon=True, framealpha=0.95)
axB.grid(alpha=0.18)

fig.tight_layout()
out = Path(__file__).resolve().parents[2] / "chapters/4.Application/images/app-rune-predict.png"
