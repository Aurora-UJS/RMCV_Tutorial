...
#   装甲板四个像素角点 → PnP(IPPE) → 相机系位姿，再送给下一章的跟踪器。
...
# ---- 输入 ----
box(1.5, 45, 13, 14, "RGB frame\n+ camera_info\n(camera module)", C_IO,
    face="#f0f0f0")

# ---- 传统级联路线（上车道）----
ax.text(17, 90, "Traditional cascade route  (CPU, sub-millisecond)",
...
# ---- CNN 四点路线（下车道）----
ax.text(17, 8.5, "CNN keypoint route  (NPU/iGPU, OpenVINO, milliseconds)",
        fontsize=9.5, color=C_CNN, fontweight="bold")
by = 16
box(17, by, 33.5, th, "YOLO four-point net\n(NHWC, LATENCY hint)", C_CNN)
box(54, by, 20, th, "sort 4 keypoints\nTL / TR / BR / BL", C_CNN)
...
# ---- 统一接口：四点 → PnP → 位姿 ----
sx, sy, sw, sh = 78, 43, 20, 18
box(sx, sy, sw, sh, "4 image points\n-> PnP (IPPE)\n-> pose in camera frame",
    C_SHARE, face="#fff4e6", fs=8.8, tw="bold")
# 两条路线汇入接口（漏斗）
arrow(87, ty, sx + sw / 2, sy + sh, C_TRAD)          # 传统路线末端 ↘
arrow(74, by + th, sx + 4, sy, C_CNN)                # CNN 路线末端 ↗

# ---- 输出 → tracker ----
ax.annotate("armors (position + yaw)\n=> tracker  (next chapter)",
...
ax.text(52, 3.7,
        "Robustness engineering: dedup shared lightbars . negative class . "
...
ax.set_title(
    "Two detection routes, one interface: both emit 4 image points -> PnP -> pose for the tracker",
    fontsize=11, pad=10)
fig.tight_layout()
out = "/home/neomelt/RMCV_Tutorial/chapters/4.Application/images/app-armor-detector-pipeline.png"
