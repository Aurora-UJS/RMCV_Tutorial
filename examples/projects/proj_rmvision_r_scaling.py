# Reproduce rm_vision's per-coordinate measurement-noise formula.
#
...
# Each variance is proportional to the absolute value of one odom coordinate,
# rather than being derived from a PnP covariance. This script sweeps a target
# along a constant-range arc and plots the resulting 1-sigma per axis, showing
# (a) sigma_x changes although the range does not and
...
F = 4e-4          # metres; f * coordinate gives a variance in m^2
...
axA.plot(ya, xa, color=GRAY, lw=1.4, ls="--", zorder=2,
         label="target path, 3-D range fixed at 4.0 m")
...
axA.set_title("Implemented odom-axis 1-sigma ellipse, drawn 20x\n"
              "magenta = line-of-sight reference; no measured covariance shown",
              fontsize=10.0)
...
axA.annotate("off-axis: the ellipse\nremains aligned with\nthe odom axes",
...
axB.axhline(np.sqrt(F * RANGE) * 1e3, color=INK, lw=1.1, ls=":",
            label=r"isotropic $\sqrt{f \cdot \mathrm{range}}$ reference = 40.0 mm")
...
axB.set_ylim(0, 50)
axB.set_xlabel("target lateral position $y_a$  [m]   (3-D range fixed at 4.0 m)")
axB.set_ylabel("assumed measurement 1-sigma  [mm]")
axB.set_title(f"At fixed range: $\\sigma_x$ = {sx_e:.1f} to {sx_c:.1f} mm "
              f"({100*(sx_c-sx_e)/sx_e:.0f}% change)\n"
              f"$\\sigma_y$ = {sy_e:.1f} to {sy[i0]*1e3:.1f} mm",
              fontsize=10.0)
axB.legend(loc="center right", fontsize=8.4, frameon=False)
axB.grid(alpha=0.25, lw=0.6)

fig.suptitle("rm_vision R = diag(|f*xa|, |f*ya|, |f*za|, r_yaw), f = 4e-4: per-coordinate scaling in odom",
             fontsize=12.0, fontweight="bold", y=1.005)
fig.tight_layout()
output = (Path(__file__).resolve().parents[2]
          / "chapters/6.Projects/images/proj-rmvision-r-scaling.png")
